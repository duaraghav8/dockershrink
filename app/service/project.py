...
This stage only includes the application code, dependencies and any other assets necessary for running the app.""",
    ...
    def _dockerfile_minimize_layers(self):
        ...
        ####################
        # This code was generated by claude dev backed by gpt 4o.
        import re

        dockerfile_content = self.dockerfile.raw()
        run_statements = re.findall(
            r"RUN\s+.*?(?=\n\S|$)", dockerfile_content, re.DOTALL
        )

        if not run_statements:
            return

        combined_run_statement = "RUN " + " && \\\n    ".join(
            [stmt.strip()[4:] for stmt in run_statements]
        )

        minimized_dockerfile_content = re.sub(
            r"RUN\s+.*?(?=\n\S|$)", "", dockerfile_content, flags=re.DOTALL
        ).strip()

        minimized_dockerfile_content = re.sub(
            r"\n{2,}", "\n", minimized_dockerfile_content
        )

        minimized_dockerfile_content = minimized_dockerfile_content.replace(
            run_statements[0], combined_run_statement, 1
        )

        self.dockerfile._raw_data = minimized_dockerfile_content
        ####################
