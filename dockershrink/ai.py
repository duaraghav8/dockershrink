...
* Perform a fresh installation of the dependencies (node_modules) in the final stage and exclude dev dependencies. Don't copy node_modules from the previous stage. If the dependencies in the previous stage were installed for the sole purpose of running the application, then remove that installation command from the previous stage since it is no longer useful. Otherwise don't change the installation commands in the previous stage.
