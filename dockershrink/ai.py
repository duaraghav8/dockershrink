...
* Perform a fresh installation of the dependencies (node_modules) in the final stage and exclude dev dependencies. Don't copy node_modules from the previous stage.
* If, after adding the final stage, the previous stage has no use for the dependencies installed in it, you should delete the dependency installation command from that stage. eg- if the previous stage installs dependencies and then immediately moves to final stage, then those dependencies are not needed. In such a case, the dependency installation command in previous stage should be removed.
